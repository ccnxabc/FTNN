...
file_souji_tmp="c:\\9\\ftnn_souji_CHAD_2.csv"
file_souji_all="c:\\9\\ftnn_souji_all_CHAD_2.csv"
file_souji_fengzhong="c:\\9\\ftnn_souji_CHAD_2_fengzhong.csv"
...
while (time.localtime(time.time()-24*60*60)<time.strptime(local_date+" "+"10:50:01", "%Y-%m-%d %H:%M:%S") \
        and time.localtime(time.time()-24*60*60)>time.strptime(local_date+" "+"09:30:00", "%Y-%m-%d %H:%M:%S")):
    #发送报价请求，Market为2表示是美股
    req = {'Protocol':'1001', 'ReqParam':{'Market':'2','StockCode':'CHAD'},'Version':'1'}
    ...
    if actual_minute==LastMinute or LastMinute==None:
        ...
        if trade_order_time==LastTradeOrderTime:
            str_tmp=time.strftime("%Y-%m-%d %H:",now_time)+str(int(actual_minute)-1)
